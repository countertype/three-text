...
function App() {
  ...
  const textControls = useControls("Text", {
    text: {
      value:
        `three-text renders and formats text in 3D, with support for Three.js, p5.js, WebGPU, and WebGL. It also exposes its core, platform-agnostic APIs for other environments. three-text works with OTF, TTF, and WOFF format font files. It uses TeX-based parameters for breaking text into paragraphs across multiple lines, and turns font outlines into 3D shapes on the fly, caching their geometries for low CPU overhead in languages with lots of repeating glyphs. Variable fonts are supported as static instances at a given axis coordinate. Under the hood, three-text relies on HarfBuzz for text shaping, Knuth-Plass line breaking, Liang hyphenation, libtess by Eric Veach for removing overlaps and triangulation, curve polygonization from Maxim Shemanarev's Anti-Grain Geometry, and Visvalingam-Whyatt line simplification`,
      rows: true,
    },
